...
public class MultiplyOperator implements TwoParamOperator {
	...
	public MultiplyOperator(int orderNum) {
	...
	public Box run(Box param1, Box param2) {
		if (param1.getOriginBox() instanceof NativeValueBox && param2.getOriginBox() instanceof NativeValueBox) {
			...
			if (object1 instanceof BigDecimal && object2 instanceof BigDecimal)	return BoxUtil.createNativeBox(((BigDecimal) object1).multiply((BigDecimal) object2), "box/native/Number.box", param1.getMomBox(), param1.getLn(), param1.getCol()).init();
			else throw new Error();
		}
		if (param1.getOriginBox().getInnerBox("multiply") != null)
			return param1.getOriginBox().getInnerBox("multiply").run(param2.getOriginBox());
		else throw new Error();
	}
}
